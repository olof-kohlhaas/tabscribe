...
public class TabsSemiIncreasableBinding extends BooleanBinding {
	
	private ObjectExpression<LinkedHashSet<TabStatement>> beatStatements;

	TabsSemiIncreasableBinding(ObjectExpression<LinkedHashSet<TabStatement>> beatStatements) {
	...
	protected boolean computeValue() {
		if (beatStatements.get() == null) {
			...
			return beatStatements.get().stream().allMatch(t -> t.isSemiIncreasable() || t.isStopTab());
		}
	}
