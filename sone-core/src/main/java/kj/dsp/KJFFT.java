...
public class KJFFT {
    ...
    private int bitrev( int j, int nu ) {
        ...
        for( int i = 1; i <= nu && !Thread.currentThread().isInterrupted(); i++ ) {
            j2 = j1 >> 1;
            k  = ( k << 1 ) + j1 - ( j2 << 1 );
            j1 = j2;
        }
    ...
    public float[] calculate( float[] pSample ) {
        ...
        for ( int a = 0, b = 0; a < pSample.length && !Thread.currentThread().isInterrupted(); a += wAps, b++ ) {
       		xre[ b ] = pSample[ a ];
        	xim[ b ] = 0.0f;
        }
        ...
        for ( int l = 1; l <= nu && !Thread.currentThread().isInterrupted(); l++ ) {
        	
            k = 0;
            
            while ( k < ss && !Thread.currentThread().isInterrupted() ) {
            	
                for ( int i = 1; i <= n2 && !Thread.currentThread().isInterrupted(); i++ ) {
        ...
        // -- Reorder output.
        for( k = 0; k < ss && !Thread.currentThread().isInterrupted(); k++ ) {
        	
        	// -- Use tabled BR values.
            r = fftBr[ k ]; 
            
            if ( r > k && !Thread.currentThread().isInterrupted()) {
        ...
        for ( int i = 1; i < ss2 && !Thread.currentThread().isInterrupted(); i++ ) {
